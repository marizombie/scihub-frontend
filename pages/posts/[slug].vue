              ...
              <v-list>
                <v-list-item v-for="(network, index) in networks" :key="index">
                  <ShareNetwork :network="network.network" :url="sharing.url" :title="sharing.title"
                    ...
                    :twitterUser="sharing.twitterUser" class="linkWOStyles d-flex align-center">
                    <v-icon v-if="network.network !== 'telegram'">
                      {{ `mdi-${network.icon}` }}
                    </v-icon>
                    <i class="v-icon notranslate theme--light v-icon--size-default" style="margin-left: -1.5px" v-else>
                      <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 54 54">
