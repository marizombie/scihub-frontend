      ...
      <div>
        ...
        <div v-if="currentShowList?.length">
          <div v-if="currentRequest === `api/upvoted-comments/`">
            <v-card
              v-for="(commentData, index) in currentShowList as CommentData[]"
              :key="index"
              class="article mb-4"
          ...
          <div v-else>
            <v-card
              v-for="(article, index) in currentShowList as Article[]"
              :key="index"
              class="article mb-8"
              @click="
                navigateTo(
      ...
      <div class="d-flex flex-column">
        <div v-if="userProfile" class="mb-12">
          <div
            ...
            @click="goToAuthorPosts(userProfile.username)"
          >
            ...
            <div class="d-flex flex-column">
              ...
              <span class="subtitle"
                >{{ userProfile.followers_count }} follower(s)</span
              >
            </div>
          </div>
          ...
          <div
            class="mt-4"
            v-if="userProfile.username !== userStore.userInfo?.username"
          >
            <v-btn
              v-if="!userProfile.is_followed_by_current_user"
              :loading="followLoading"
              @click="followAuthor(userProfile.username)"
              color="primary"
            >
              Follow
            </v-btn>
        ...
        >
          ...
          <div
            class="author-info"
            @click.prevent="goToAuthorPosts(item.author_name)"
          >
            <span class="mr-1">by</span>
            <v-avatar size="20" class="mr-1">
              <v-img
...
useInfiniteScroll(document, loadData, { distance: 200 });
...
async function showTagList() {
  if (route.query.tag) {
    filterByTags.value = Array.isArray(route.query.tag)
      ...
      : [route.query.tag];
    const { data: followed } = await useAPIFetch<SuccessResponse>(
      `api/is-tag-followed/${filterByTags.value}/`
    );
    if (typeof followed.value?.success === 'boolean') {
      isFollowedCurrentTag.value = followed.value.success;
    }
    const { data: tagPosts } = await useAPIFetch<CRUDResponse>(
      `/api/tags/${filterByTags.value}/?limit=5&offset=0`
    );
...
async function goToComment(commentData: CommentData) {
  if (commentData) {
    await navigateTo(
      `/posts/${commentData.post_slug}?comment_id=${commentData.id}`
    );
  }
}
...
async function onDeletePost() {
  const { error } = await useAPIFetch<CommentData[]>(
    deletePostSlug.value
      ...
      : `api/drafts/${deleteDraftSlug.value}/`,
    {
      method: 'delete'
    }
  );
  if (error.value) {
    ...
    currentShowList.value = (currentShowList.value as Article[]).filter(
      (item: Article) =>
        deletePostSlug.value
          ? item.slug !== deletePostSlug.value
          : item.slug !== deleteDraftSlug.value
    );
    deletePostSlug.value = '';
...
async function followAuthor(name: string) {
  ...
  followLoading.value = false;
  userProfile.value!.is_followed_by_current_user =
    !userProfile.value!.is_followed_by_current_user;
}
