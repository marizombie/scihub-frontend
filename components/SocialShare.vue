  ...
  <v-menu offset-y>
    <template v-slot:activator="{ isActive, props }">
      ...
      >
        ...
        <v-tooltip v-if="!isActive" activator="parent" location="bottom"
          >Share</v-tooltip
        ></v-btn
      >
    </template>
    <v-list>
      <v-list-item
        ...
        :key="index"
        link
        @click="selectItem(network)"
      >
        <v-icon
          v-if="network.type !== 'telegram' && network.type !== 'twitter'"
        >
          {{ `mdi-${network.icon}` }}
        </v-icon>
        <i
          class="v-icon notranslate theme--light v-icon--size-default"
          v-else-if="network.type === 'telegram'"
        >
          <svg
...
async function selectItem(item: Network) {
  switch (item.type) {
    ...
    case 'email':
      ...
      let bodyMessage = encodeURIComponent(
        'Look what I found at ' + window.location.href
      );
      emailElem.setAttribute(
        'href',
        `mailto:?subject=${subjectEncoded}&body=${bodyMessage}`
      );
    ...
    case 'linkedin':
      ...
      linkedinElem.setAttribute(
        'href',
        `http://www.linkedin.com/shareArticle?mini=true&url=${window.location.href}&title=${bodyTitleEncoded}&source=${window.location.host}`
      );
    ...
    case 'telegram':
      ...
      telegramElem.setAttribute(
        'href',
        `https://t.me/share?url=${window.location.href}&text=Check%20this%20out%21`
      );
    ...
    case 'twitter':
      ...
      xElem.setAttribute(
        'href',
        `https://twitter.com/intent/tweet?text=${shareObject.title}&url=${window.location.href}`
      );
