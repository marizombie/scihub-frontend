      ...
      <v-card>
        <form @submit.prevent="onSubmit">
          <v-card-title class="mt-4">
            <span class="text-h5 pl-6">{{ t("resendLink") }}</span>
          </v-card-title>
...
import type { UserInfo, ErrorKeyValue, SuccessResponse } from "~/types";
const { t } = useI18n({
  useScope: "local",
});
...
const onSubmit = handleSubmit(async (values) => {
  ...
  const { data, error } = await useAPIFetch<SuccessResponse>("/api/register/resend-email/", {
    method: "post",
